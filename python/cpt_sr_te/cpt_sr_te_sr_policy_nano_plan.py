...
from cisco_tsdn_core_fp_common.utils import (
    validate_service,
    update_plan_status_codes,
    update_component_status_codes,
    remove_status_code_detail,
    is_cq_enabled,
...
class ServiceCallback(ncs.application.Service):
    ...
    def cb_post_modification(self, tctx, op, kp, root, proplist):
        ...
        if op == NCS_SERVICE_UPDATE:
            ...
            if not is_redeploy:
                ...
                sr_te_policy_kp = (
                    "/cisco-sr-te-cfp:sr-te/cisco-sr-te-cfp-sr-policies:"
                ...
                device_cq_details = is_cq_enabled(
                    self, root, updated_devices, th, utils.is_lsa_setup()
                )
                ...
                if th.exists(plan_kp):
                    ...
                    for device, cq_enabled in device_cq_details:
                        ...
                        if cq_enabled:
                            plan.component[
                                ("cpt-sr-te-sr-policy-nano-plan:head-end", device)
                            ].state["ncs:ready"].status = "not-reached"
                            plan.component[("ncs:self", "self")].state[
                                "ncs:ready"
                            ].status = "not-reached"
                        ...
                        # for both device component and self
                            ...
                            update_state_when_timestamp(
                                self,
                                plan.component[("ncs:self", "self")].name,
    ...
    def diff_iterate_redeploy(th, service) -> DiffIterateWrapper:
        ...
        redeploy_kp = (
            f"/cpt-sr-te:sr-policy{{{service.name}}}/" "private/re-deploy-counter"
        )
...
class SelfCallback(ncs.application.NanoService):
    ...
    def _create_ready(
        ...
        internal_plan_list = (
            root.cisco_sr_te_cfp__sr_te.cisco_sr_te_cfp_sr_policies__policies.policy_plan
        )
        ...
        if internal_plan_list.exists(internal_service_name):
            ...
            if (
                sr_te_cfp_plan.component.exists(self_key)
                and sr_te_cfp_plan.component[self_key].state["ncs:ready"].status
                == "failed"
            ) or sr_te_cfp_plan.failed:
            ...
            if not (
                ...
                for plan_state in plan_component.state:
                    if plan_state.status == "failed":
                        self.log.info(
                            f"Component '{plan_component.name}' state '{plan_state.name}'"
        ...
        for plan_component in plan.component:
            if not (
                ...
                if plan_component.state["ncs:ready"].status == "not-reached":
                    self.log.info(
                        f"Component '{plan_component.name}' state ncs:ready not-reached, "
...
class HeadEndCallback(ncs.application.NanoService):
    ...
    def _create_config_apply(
        ...
        tail_end = utils.get_loopback0_ip_address(
            service.tail_end, err_log=self.log.error
        )
    ...
    def _create_ready(
        ...
        if sr_te_cfp_service_name not in sr_te_cfp_plan_list:
            self.log.info(
                f"cisco-sr-te-cfp policy-plan doesn't exist for {sr_te_cfp_service_name}"
            )
            state_node.status = "not-reached"
            return opaque
        ...
        if sr_te_cfp_plan.failed:
            self.log.debug(
                f"cisco-sr-te-cfp policy-plan {sr_te_cfp_service_name} failed, marking "
